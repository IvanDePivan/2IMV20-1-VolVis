...
public class TransferFunctionView extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int value : histogram) {
            maxBinHeigth = Math.max(value, maxBinHeigth);
        }
        ...
        int xprev = -1;
        int yprev = -1;
        for (TransferFunction.ControlPoint cp : controlPoints) {
            int s = cp.value;
    ...
    private class ControlPointHandler extends MouseMotionAdapter {
        ...
        public void mouseMoved(MouseEvent e) {
            ...
            for (TransferFunction.ControlPoint controlPoint : controlPoints) {
                inside = inside || getControlPointArea(controlPoint).contains(e.getPoint());
            }
            if (inside) {
    ...
    private class SelectionHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            while (idx < controlPoints.size()) {
                inside = getControlPointArea(controlPoints.get(idx)).contains(e.getPoint());
                if (inside) {
                    break;
                } else {
                    idx++;
